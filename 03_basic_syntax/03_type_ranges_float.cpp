...
int main(int, char *[]) {
	...
	cout << endl
		 << setw(text_width + 1) << "Epsilon = " << std::numeric_limits<float>::epsilon() << endl;
	;
	...
	cout << endl
		 << setw(text_width + 1) << "Epsilon = " << std::numeric_limits<double>::epsilon() << endl;
	...
	cout << endl
		 << setw(text_width + 1) << "Epsilon = " << std::numeric_limits<long double>::epsilon() << endl;
	cout << endl;
	...
	if (std::fabs(a - b) < myEpsilon) {
		cout << "a == b within " << myEpsilon << std::endl;
	}
	
	{
		const auto doubleEpsilon = std::numeric_limits<double>::epsilon();
		cout << endl
			<< "Epsilon for double:" << std::endl
